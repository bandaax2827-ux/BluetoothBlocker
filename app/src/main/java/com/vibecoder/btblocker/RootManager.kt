...
object RootManager {
    ...
    // ЭТОТ МЕТОД БЫЛ ОТСУТСТВЕН, ИЗ-ЗА ЧЕГО БЫЛА ОШИБКА
    fun hasPrivileges(context: Context): Boolean {
        return checkRoot() || ShizukuManager.isShizukuRunning()
    }

    fun run(cmd: String): Boolean {
        // Сначала пробуем Shizuku
        if (ShizukuManager.isShizukuRunning()) {
            return ShizukuManager.runCommand(cmd)
        }
        
        // Если Shizuku нет, пробуем root
    ...
    fun enableAllBluetoothPackages() {
        ...
        run("am force-stop com.android.bluetooth")
    }
}
